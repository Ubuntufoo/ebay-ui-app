...
function isGroup(value: unknown, id: string): value is VariationListingGroup {
  ...
  return g.groupId === id && (g.listingId === null || typeof g.listingId === "string") && (g.listingUrl === null || typeof g.listingUrl === "string") && Number.isInteger(g.desiredRevision) && typeof g.lifecycleState === "string" &&
...
export function VariationPublicationPanel({group, capturePending, onGroupUpdated, onActionSettled}: Props) {
  ...
  const reconcileReady = !capturePending && !runningAction && !actionStateLocked && recovery?.retryStatus === "reconciliation_required" && recovery.requiresReconciliation && recovery.reconciliationSupported === true;
  const reconcileUnavailable = recovery?.retryStatus === "reconciliation_required" && recovery.requiresReconciliation && recovery.reconciliationSupported === false;
  ...
  return <section className="rounded-[1.5rem] border border-stone-950/10 bg-white/90 p-3">
    ...
    {group.validation.blockers.length > 0 || recovery && recovery.retryStatus !== "not_applicable" ? (
      <div className="mt-3 grid gap-3 md:grid-cols-2">
        {group.validation.blockers.length > 0 ? (
          <div className={`rounded-xl bg-rose-50 px-4 py-3 text-sm text-rose-900 ${recovery && recovery.retryStatus !== "not_applicable" ? "" : "md:col-span-2"}`}>
            <p className="font-bold">Publication blockers</p>
            <ul className="mt-2 list-disc space-y-1 pl-5 text-xs">
              {group.validation.blockers.map((blocker) => <li key={blocker}>{blocker}</li>)}
            </ul>
          </div>
        ) : null}
        {recovery && recovery.retryStatus !== "not_applicable" ? (
          <div className={`rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-xs text-rose-900 ${group.validation.blockers.length > 0 ? "" : "md:col-span-2"}`}>
            <p className="font-bold">Recovery: {token(recovery.retryStatus)}</p>
            <p className="mt-1">Stage remote state: {token(recovery.remoteState)} · {recovery.requiresReconciliation ? "reconciliation required" : "reconciliation not required"}</p>
            <p className="mt-1">Revision: {recovery.revisionId}{recovery.operationKey ? ` · operation: ${recovery.operationKey}` : ""}</p>
            <p className="mt-1">Recommended: {recovery.recommendedActions.join(", ") || "inspect durable history"}</p>
            {reconcileUnavailable ? <p className="mt-1 font-semibold">This revision kind cannot be reconciled from this workspace. Inspect remote state and resolve it manually before any retry.</p> : null}
          </div>
        ) : null}
      </div>
    ) : null}
