...
export function VariationPublicationPanel({group, capturePending, onGroupUpdated, onActionSettled}: Props) {
  ...
  const listingLinkLabel = listingUrl?.includes("sandbox.ebay.com") ? "View on eBay Sandbox" : "View on eBay";
  return <section className="rounded-[1.5rem] border border-stone-950/10 bg-white/90 p-5">
    ...
    {confirmed ? <p className="mt-3 rounded-xl bg-stone-50 px-3 py-2 text-xs text-stone-700">For published groups, eBay is authoritative for live quantity and order history. Use Withdraw to end the listing; destructive cleanup is reserved for unpublished recovery.</p> : null}
    {listingUrl ? <p className="mt-3 text-sm"><a href={listingUrl} target="_blank" rel="noreferrer noopener" className="font-semibold text-sky-700 underline decoration-sky-300 underline-offset-2">{listingLinkLabel}</a></p> : null}
